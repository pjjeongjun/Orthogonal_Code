function mahal_dist = mahalanobis_distance3(W, firingRates_train, firingRates_test, nComponents, task_components)

% Decode the 2nd target direction
...
if isempty(find(~isnan(Xfull_test)==0)) %if all cells have response to all directions and times
    ...
    for direction_dist = 1:size(Zfull,3)
        for direction_test = 1:size(Zfull_test,3)
