function [Xcen, Z, dataDim] = dpca_plot(Xfull, W, V, plotFunction, varargin)
...
if ~isempty(options.whichMarg) && length(unique(options.whichMarg)) <= 4% && length(unique(options.whichMarg)) > 1
