function mahal_dist = mahalanobis_distance(W, firingRates_train, firingRates_test, nComponents, task_components)
    ...
    for direction_dist = 1:size(Zfull,2)
        for direction_test = 1:size(Zfull_test,2)
            for time = 1:size(Zfull,3)
                ...
                if nComponents == 1                 
                    mahal_dist(direction_dist,direction_test,time) = mahal(test_trial,train_trials);
                else
                    mahal_dist(direction_dist,direction_test,time) = mahal(test_trial',train_trials');
                end
            end
