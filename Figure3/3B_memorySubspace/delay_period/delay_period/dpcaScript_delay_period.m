...
for task = 1:3 % 1:nolook, 2:look, 3:combined
    ...
    if task == 1 %no-look
        firingRates = squeeze(firingRates(:,:,1,period,:));
    elseif task == 2 %look
        firingRates = squeeze(firingRates(:,:,2,period,:));
    elseif task == 3 % combined
        nolook = squeeze(firingRates(:,:,1,period,:));
        look = squeeze(firingRates(:,:,2,period,:));
        ...
        for n = 1:size(firingRates_combined,1)
            for d = 1:size(firingRates_combined,2)
                % % use all trials
                % firingRates_combined(n,d,:,1:nolook_idx(n,d)+look_idx(n,d)) = cat(4, nolook(n,d,:,1:nolook_idx(n,d)),look(n,d,:,1:look_idx(n,d)));
