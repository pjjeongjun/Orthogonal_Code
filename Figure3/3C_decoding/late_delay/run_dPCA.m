...
% Time selection
period = 1:10;  %500~1000 ms (late delay)
% This data contains 500:50:1000 ms from target offset
...
for shifted = 1:60
    ...
    for task = 1:3 %1:nolook 2:look %3:mixed
        load(['firingRates_directions_' num2str(shifted) '_normalized.mat']); % cell x direction x task x time x trial
        if task == 3
            firingRates_nolook = squeeze(firingRates(:,:,1,period,:));
            firingRates_look = squeeze(firingRates(:,:,2,period,:));
            firingRates_task = cat(4,firingRates_nolook,firingRates_look);
        else
            firingRates_task = squeeze(firingRates(:,:,task,period,:));
        end
        ...
        for boots = 1:nboots
            for n = 1:size(firingRates_task,1)
                if task == 3
                    ...
                    trainset_nolook(n,:,:,:) = firingRates_nolook(n,:,:,train_nolook);
                    testset_nolook(n,:,:) = squeeze(firingRates_nolook(n,:,:,test_nolook));
                    ...
                    trainset_look(n,:,:,:) = firingRates_look(n,:,:,train_look);
                    testset_look(n,:,:) = squeeze(firingRates_look(n,:,:,test_look));     

                    %combined data
                    trainset = cat(4,trainset_nolook,trainset_look);
                    testset = cat(4,testset_nolook,testset_look);

                else
                    mintrials = [];
                    for d = 1:size(firingRates_task,2)
                        mintrials = [mintrials find(isnan(firingRates_task(n,d,3,:)),1)];
                    end
                    min_trial = min(mintrials)-1;

                    % if min_trial < 10
                    %     keyboard
                    % end

                    test = randsample(min_trial,1);
                    trials = 1:min_trial; trials(test) = [];
                    train = randsample(trials,ntrials,true);
                    % train = randsample(trials,ntrials);

                    trainset(n,:,:,:) = firingRates_task(n,:,:,train);
                    testset(n,:,:) = squeeze(firingRates_task(n,:,:,test));
                end
            end
            ...
            firingRatesAverage = squeeze(mean(firingRates,4));
            ...
            componentsToUse = find(whichMarg == 1);
            ...
            componentsToUse_tt{boots,task} = componentsToUse;
            testset_tt{boots,task} = testset;

            disp(['Task' num2str(task) ', iteration ' num2str(boots) '/' num2str(nboots) '']);
        end
    end
    save(['regul_train_test_' num2str(shifted) '.mat'],'W_tt','V_tt','firingRates_tt','firingRatesAverage_tt','componentsToUse_tt','testset_tt','-v7.3');
end